...
export function SectionTitle({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div className="flex items-end gap-3 border-b border-border pb-3">
      ...
      {subtitle && <span className="pb-0.5 text-[10px] text-muted-foreground md:text-xs">{subtitle}</span>}
    </div>
  );
}

export function Panel({ children }: { children: ReactNode }) {
  return <div className="min-h-full space-y-4 p-4 md:p-5">{children}</div>;
}
